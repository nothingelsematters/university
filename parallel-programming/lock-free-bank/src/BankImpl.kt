...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
         ...
         */
        require(amount > 0) { "Invalid amount: $amount" }
        check(amount >= 0) { "Underflow" }
        while (true) {
            val currentAccount = account(index)
            if (currentAccount.invokeOperation()) continue
            check(currentAccount.amount - amount >= 0) { "Underflow" }
            val updated = Account(currentAccount.amount - amount)
            if (accounts[index].compareAndSet(currentAccount, updated)) return updated.amount
        }
    }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val minAcquired = acquire(minIndex, this)
            val maxAcquired = acquire(maxIndex, this)

            if (minAcquired != null && maxAcquired != null) {
                var fromAccount = if (minIndex == fromIndex) minAcquired else maxAcquired
                var toAccount = if (minIndex == toIndex) minAcquired else maxAcquired
