...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException, NoSuchAlgorithmException {
        ...
        if (file.isDirectory()) {
            file = new File(root, request.getUri() + '/' + "index.html");
        }
        if (file.isFile()) {
            MessageDigest md = MessageDigest.getInstance("MD5");
            md.update(readFile(file));
            String newHash = DatatypeConverter.printHexBinary(md.digest()).toUpperCase();
    ...
    private static final class Request {
        ...
        private Request(String request) throws IOException {
